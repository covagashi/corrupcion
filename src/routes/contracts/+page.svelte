<script lang="ts">
	...
	import { pesoShort, percent } from '$lib/format';

	let { data }: { data: PageData } = $props();

	// Link that flips the flagged-only filter, preserving search / source / province.
	const toggleHref = $derived.by(() => {
		...
		if (data.province) parts.push(`province=${encodeURIComponent(data.province)}`);
		const qs = parts.join('&');
		return resolve(qs ? `/contracts?${qs}` : '/contracts');
	});

	// Same filters minus the province, for the "clear area" link.
	const clearProvinceHref = $derived.by(() => {
		const parts: string[] = [];
		if (!data.flaggedOnly) parts.push('all=1');
		if (data.search) parts.push(`q=${encodeURIComponent(data.search)}`);
		if (data.source) parts.push(`source=${data.source}`);
		const qs = parts.join('&');
		return resolve(qs ? `/contracts?${qs}` : '/contracts');
...
<svelte:head>
	<title
		>{data.province ? `${data.province} — government contracts` : 'Government contracts'} — irregularity
		check</title
	>
	<meta
		name="description"
		content="Philippine government contracts (flood control, DPWH, PhilGEPS) ranked by simple, auditable irregularity flags."
	/>
</svelte:head>
...
<main class="mx-auto max-w-screen-sm px-4 pb-16">
	...
	<header class="pb-2">
		<h1 class="text-xl font-bold text-slate-900">
			{data.province ? data.province : 'Government contracts'}
		</h1>
		{#if data.province}
			<p class="mt-1 text-sm text-slate-600">
				Contracts in {data.province}, most concerning first.
				<a class="text-blue-700 underline" href={clearProvinceHref}>Clear area</a>
			</p>
		{:else}
			<p class="mt-1 text-sm text-slate-600">
				{data.totals.contracts.toLocaleString()} contracts worth {pesoShort(
	...
	<form method="GET" class="mb-4 flex flex-wrap gap-2">
		...
		{#if data.province}<input type="hidden" name="province" value={data.province} />{/if}
		<select
			name="source"
			value={data.source ?? ''}
